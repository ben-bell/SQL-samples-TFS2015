...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[WorkItemsLatestAndWere]'))
DROP VIEW dbo.WorkItemsLatestAndWere
GO

CREATE VIEW dbo.WorkItemsLatestAndWere
...
FROM
		dbo.vw_WorkItemCoreAll WL WITH (NOLOCK)
